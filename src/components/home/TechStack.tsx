...
// Two rows for the marquee
const row1 = techCategories.slice(0, 3).flatMap((c) => c.items);
const row2 = techCategories.slice(3).flatMap((c) => c.items);

function TechChip({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full glass border border-navy-500/50 text-sm text-[#94A3B8] font-mono whitespace-nowrap hover:border-blue-600/40 hover:text-blue-300 transition-colors cursor-default">
      ...
      <AnimatedSection className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16" delay={0.2}>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {techCategories.map((cat) => (
            <div key={cat.label} className="glass rounded-xl p-4">
              <div className="text-xs font-mono text-blue-400 tracking-wider uppercase mb-3">
                {cat.label}
              </div>
